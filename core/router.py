...
def init_db():
    ...
    if cursor.fetchone()[0] == 0:
        default_presets = [
            ("Dev Copilot", "You are Kiemaen AI, an advanced local-first personal AI workstation running inside Termux. Be concise, technical, and precise."),
            ("Structural Tutor", "You are an expert civil engineering tutor specializing in structural mechanics, beam deflection, and numerical methods. Explain step-by-step with clear math formulas."),
            ("Python Refactorer", "You are a senior Python architect. Review code for performance, security flaws, and idiomatic correctness.")
        ]
        cursor.executemany("INSERT OR IGNORE INTO presets (name, prompt) VALUES (?, ?)", default_presets)
    conn.commit()
    conn.close()
...
async def chat_endpoint(req: ChatRequest):
    ...
    try:
        router_completion = client.chat.completions.create(
            model=req.model,
            messages=[
                {"role": "system", "content": "Analyze prompt. Reply with ONLY CODE if it needs code generation/execution, else REASONING."},
    ...
    if "CODE" in route:
        try:
            code_res = client.chat.completions.create(
                model=req.model,
                messages=messages + [{"role": "user", "content": "Write clean executable Python code inside ```python ``` block."}],
                temperature=0.5
            )
...
async def rename_session(session_id: str, req: RenameRequest):
    ...
    conn = sqlite3.connect("chat.db")
    cursor = conn.cursor()
    cursor.execute("UPDATE messages SET session_id = ? WHERE session_id = ?", (new_id, session_id))
    conn.commit()
    conn.close()
