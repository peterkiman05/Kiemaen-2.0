...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🤖 Multi-Agent System Live!\nUsage: /trade EURUSD=X 15m Trend-Following Strategy")

async def trade(update: Update, context: ContextTypes.DEFAULT_TYPE):
    task_text = " ".join(context.args) if context.args else "EURUSD=X 15m scalp trading strategy"
    
    status_msg = await update.message.reply_text("⏳ Agent pipeline executing... (Fetching live market data & running Worker ➔ Reviewer loop)")
